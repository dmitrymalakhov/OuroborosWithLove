...
| Telegram uploads | runtime upload pipeline + `analyze_document` | Telegram document attachments are stored in `uploads/YYYY-MM-DD/` inside the sender's workspace, then passed to the agent as a readable path. ZIP uploads can be analyzed directly or unpacked first. |
